...
public class SwitchPreference extends TwoStatePreference {
    ...
    private static Field mCanRecycleLayoutField;
    ...
    */
    ...
    public SwitchPreference(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //Begin: Ossdk for Switch Animation @{
        try {
            if (mCanRecycleLayoutField == null) {
                if (Build.VERSION.SDK_INT >= 19) {
                    mCanRecycleLayoutField = android.preference.Preference.class.getDeclaredField("mCanRecycleLayout");
                } else {
                    mCanRecycleLayoutField = android.preference.Preference.class.getDeclaredField("mHasSpecifiedLayout");
                }
                mCanRecycleLayoutField.setAccessible(true);
            }
            mCanRecycleLayoutField.set(this, Build.VERSION.SDK_INT >= 19);
        } catch (Exception e) {
            e.printStackTrace();
        }
        //End : Ossdk for Switch Animation @}
        ...
        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.TalpaOssdkSwitchPreference, defStyleAttr, 0);
        a.recycle();
    }
    ...
    public SwitchPreference(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.Ossdk_SwitchPreferenceStyle);
    }
     ...
     */
    //Begin: Ossdk for Switch Animation @{ Copy From TwoStatePreference.java
    ...
    private void performPreferenceChanged() {
        try {
            if (sOnPreferenceChanged == null) {
                sOnPreferenceChanged = SwitchPreference.class.getMethod("onPreferenceChange");
                sOnPreferenceChanged.setAccessible(true);
            }
            sOnPreferenceChanged.invoke(SwitchPreference.this);
        } catch (Exception e) {

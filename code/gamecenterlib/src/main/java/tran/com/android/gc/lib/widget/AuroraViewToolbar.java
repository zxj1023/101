...
//                mWindowManager.removeView(mToolbarView);
    ...
    private void prepare(int screenX, int screenY, int cursorLineHeight, boolean selected) {
        ...
        if (aboveCursor) {
            ...
            //mToolbarGroup.setPadding(2, 2, 2, 2);
            mToolbarPositionArrowView.setPadding(mEditBarArrowPaddingLeft, mToolbarGroup.getMeasuredHeight() - mArrowBelowDrawable.getIntrinsicHeight()*2+DensityUtil.dip2px(mContext, 0.5f),
            ...
            mToolbarPositionArrowView.setPadding(mEditBarArrowPaddingLeft, mOffsetToolbar+mArrowBelowDrawable.getIntrinsicHeight()-DensityUtil.dip2px(mContext, 0.5f), mEditBarArrowPaddingRight, 0);
        }
        //gionee 20121210 guoyx modified for CR00734816 end

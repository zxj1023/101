...
public interface AuroraMenuView {
	/**
	...
	/**
	 * Minimal interface for a menu item view. {@link #initialize(MenuItemImpl, int)} must be called for the
	...
	public interface ItemView {
		 ...
		 * @param menuType
		 *            The type of this menu, one of {@link MenuBuilder#TYPE_ICON},
		 *            {@link MenuBuilder#TYPE_EXPANDED}, {@link MenuBuilder#TYPE_DIALOG}).
		 */
		...
		/**
		 * Displays the checkbox for the item view. This does not ensure the item view will be checked, for
		 * that use {@link #setChecked}.
		 * 
		 * @param checkable
		...
		/**
		 * Checks the checkbox for the item view. If the checkbox is hidden, it will NOT be made visible, call
		 * {@link #setCheckable(boolean)} for that.
		 * 
		 * @param checked
		 ...
		 * 
		 * @param showShortcut
		 *            Whether a shortcut should be shown(if false, the value of shortcutKey should be
		 *            ignored).
		 * @param shortcutKey
		...
		/**
		 * Whether this item view prefers displaying the condensed title rather than the normal title. If a
		 * condensed title is not available, the normal title will be used.
		 * 

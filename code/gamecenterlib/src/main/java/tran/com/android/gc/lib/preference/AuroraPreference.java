...
import tran.com.android.gc.lib.widget.AuroraSpinner;
...
import tran.com.android.gc.lib.utils.AuroraCharSequences;
import tran.com.android.gc.lib.utils.DensityUtil;

public class AuroraPreference implements Comparable<AuroraPreference>, OnDependencyChangeListener {
    /**
    ...
    private int mLayoutResId = R.layout.aurora_preference;
    private int mWidgetLayoutResId = R.layout.aurora_preference_widget_arrow;

    // add by <flash>2013.10.16 start
    private int auroraBottomLayoutResId = 0;
    // end
    ...
    private List<AuroraPreference> mDependents;

    private boolean mBaseMethodCalled;

    // Aurora <Luofu> <2013-10-25> modify for status begin
    ...
    private ColorStateList mSummaryColorList;
    private ColorStateList mStatusColorList;
    
    private boolean mSetTitleColorList;
    private boolean mSetSummaryColorList;
    private boolean mSetStatusColorList;
    
    private boolean mSetTitleColor;
    private boolean mSetSummaryColor;
    private boolean mSetStatusColor;
    
    private int mTitleColorRes;
    private int mSummaryColorRes;
    private int mArrowTextColorRes;
    private int mArrowBgRes;
    // Aurora <Luofu> <2013-10-25> modify for status end
    
    
    private int mIconWidth;
    private int mIconHeight;
    
    private int mMinHeightWithSummary = 64;//dp
    private int mEmptyHeight = 20;//dp
    
    int mPreferenceHeight;
    /**
    ...
    public interface OnPreferenceChangeListener {
         ...
         *         value.
         */
        boolean onPreferenceChange(AuroraPreference preference, Object newValue);
    }
    ...
    public interface OnPreferenceClickListener {
         ...
         */
        boolean onPreferenceClick(AuroraPreference preference);
    }
    ...
    interface OnPreferenceChangeInternalListener {
         ...
         */
        void onPreferenceChange(AuroraPreference preference);
         ...
         * @param preference This Preference.
         */
        void onPreferenceHierarchyChange(AuroraPreference preference);
    }
     ...
     *            preference.
     * @param defStyle The default style to apply to this preference. If 0, no
     ...
     */
    public AuroraPreference(Context context, AttributeSet attrs, int defStyle) {
        mContext = context;

        // Gionee <zhangxx> <2013-08-14> modify for CR00857086 begin
        if (NativePreferenceManager.getAnalyzeNativePreferenceXml() && attrs != null) {
            AnalyzeNativePreferenceAttributeSet(context, attrs, defStyle);
        } else {
            AnalyzeAuroraPreferenceAttributeSet(context, attrs, defStyle);
        }
        // Gionee <zhangxx> <2013-08-14> modifyfor CR00857086 end

        if (!getClass().getName().startsWith("tran.com.android.gc.lib.preference")) {
            // For subclasses not in this package, assume the worst and don't
            // cache views
            mHasSpecifiedLayout = true;
        }
        mWidgetPaddingRight = DensityUtil.dip2px(mContext, 15f);        
       mPreferenceHeight = DensityUtil.dip2px(mContext, 14.0f);
    }
     ...
     */
    public AuroraPreference(Context context, AttributeSet attrs) {
     ...
     */
    public AuroraPreference(Context context) {
        this(context, null);
    }
     ...
     * 
     * @return The {@link Intent} last set via {@link #setIntent(Intent)} or
     *         XML.
     */
    ...
    /**
     * Sets the class name of a fragment to be shown when this Preference is
     * clicked.
     * 
     * @param fragment The class name of the fragment associated with this
     *            Preference.
     */
    ...
    public void setTitleColor(ColorStateList color){
        mTitleColorList = color;
        mSetTitleColorList = true;
    }
    
    public void setSummaryColor(int color){
        mSummaryColor = color;
        mSetSummaryColor = true;
    }
    public void setSummaryColor(ColorStateList color){
        mSummaryColorList = color;
        mSetSummaryColorList = true;
    }
    
    public void setStatusColor(ColorStateList color){
        mStatusColorList = color;
        mSetStatusColorList = true;
    }
    
    public void setStatusColor(int color){
        mStatusColor = color;
        mSetStatusColor = true;
    }
    public ColorStateList getTitleColorList(){
        return mTitleColorList;
    }
    public ColorStateList getSummaryColorList(){
        return mSummaryColorList;
    }
    public ColorStateList getStatusColorList(){
        return mStatusColorList;
    }
    public boolean getSetTitleColorList(){
        return mSetTitleColorList;
    }
    public boolean getSetSummaryColorList(){
        return mSetSummaryColorList;
    }
    public boolean getSetStatusColorList(){
        return mSetStatusColorList;
    }
    public boolean getSetTitleColor(){
        return mSetTitleColor;
    }
    public boolean getSetSummaryColor(){
        return mSetSummaryColor;
    }
    public boolean getSetStatusColor(){
        return mSetStatusColor;
    }
     ...
     * {@link R.id#widget_frame} to be the parent of the specific widget for
     * this Preference. It should similarly contain {@link R.id#title} and
     * {@link R.id#summary}.
     * 
    ...
    /**
     * Gets the layout resource that will be shown as the {@link View} for this
     * Preference.
     * 
     ...
     * This is inflated into the main layout. For example, a
     * {@link CheckBoxPreference} would specify a custom layout (consisting of
     * just the CheckBox) here, instead of creating its own main layout.
     * 
    ...
    public void setWidgetlayoutPadding(int paddingLeft,int paddingTop,int paddingRight,int paddingBottom){
    	mWidgetPaddingLeft = paddingLeft;
    	mWidgetPaddingRight = paddingRight;
    	mWidgetPaddingTop = paddingTop;
    	mWidgetPaddingBottom = paddingBottom;
    }
    private boolean contentSpinner(View view){
    	if(view == null){
    		return false;
    	}
    	if(view instanceof AuroraSpinner){
    		((AuroraSpinner)view).setPosition(AuroraSpinner.POSITION_RIGHT_TOP);
    		return true;
    	}
    ...
    protected View onCreateView(ViewGroup parent) {
        ...
        Log.e("zxx", "widgetFrame = " + (widgetFrame != null));
        if (widgetFrame != null) {
            if (mWidgetLayoutResId != 0) {
                layoutInflater.inflate(mWidgetLayoutResId, widgetFrame);
                if(contentSpinner(widgetFrame)){
                	widgetFrame.setPadding(mWidgetPaddingLeft, mWidgetPaddingTop, 0, mWidgetPaddingBottom);
                }else{
                	widgetFrame.setPadding(mWidgetPaddingLeft, mWidgetPaddingTop, mWidgetPaddingRight, mWidgetPaddingBottom);
                }
                View arrow = widgetFrame.findViewById(R.id.aurora_arrow_inner);
                if ((this instanceof AuroraPreferenceScreen) || mIntent != null
                        || mFragment != null) {
                    if (arrow != null) {
        ...
        // add by <flash>2013.10.16
        final ViewGroup aurorabottomwidgetFrame = (ViewGroup) layout
                .findViewById(R.id.aurora_bottom_widget_frame);

        final ImageView auroraGap = (ImageView) layout
                .findViewById(R.id.aurora_bottom_gap_line);
        if (aurorabottomwidgetFrame != null)
        {
            if (auroraBottomLayoutResId != 0)
            {
                layoutInflater.inflate(auroraBottomLayoutResId, aurorabottomwidgetFrame);
            }
            else
            {
                auroraGap.setVisibility(View.GONE);
                aurorabottomwidgetFrame.setVisibility(View.GONE);
            }
        }
        // end
    ...
    private void setPaddingForPreference(View parent){
        if(parent == null){
            return;
        }
        boolean setPadding  = false;
        CharSequence summary = getSummary();
        if(this instanceof AuroraTwoStatePreference){
            final CharSequence summaryOn = ((AuroraTwoStatePreference)this).getSummaryOn();
            final CharSequence summaryOff = ((AuroraTwoStatePreference)this).getSummaryOff();
            if((TextUtils.isEmpty(summaryOn) && TextUtils.isEmpty(summaryOff)) && TextUtils.isEmpty(summary)){
                setPadding = true;
        ...
        if(setPadding){
            parent.setPadding(parent.getPaddingLeft(),
                    DensityUtil.dip2px(mContext, 2), parent.getPaddingRight(), 
                    0);
        }
    }
    ...
    protected void onBindView(View view) {
        ...
        if (titleView != null) {
            ...
            final ColorStateList titleColorList = getTitleColorList();
            final boolean setColor = getSetTitleColor();
            final boolean setColorList = getSetTitleColorList();
            if (!TextUtils.isEmpty(title)) {
                titleView.setText(title);
                if(setColor){
                    titleView.setTextColor(titleColor);
                }
                if(setColorList && (titleColorList != null)){
                    titleView.setTextColor(titleColorList);
                }
         ...
         if(this instanceof AuroraPreferenceCategory){
             final TextView category_title = (TextView) view.findViewById(
                     R.id.aurora_preference_category_title);
             if (category_title != null) {
                 final CharSequence title = getTitle();
                 if (!TextUtils.isEmpty(title)) {
                     category_title.setText(title);
                 } else {
                     int categoryHeight = mPreferenceHeight;
                     LinearLayout layout = (LinearLayout)view.findViewById(R.id.aurora_preference_category_layout);
                     layout.setLayoutParams(new LinearLayout.LayoutParams(
                             LinearLayout.LayoutParams.MATCH_PARENT, mPreferenceHeight));
                 }
             }
        ...
        if (summaryView != null) {
            ...
            final int summaryColor = getSummaryColor();
            final ColorStateList summaryColorList = getSummaryColorList();
            final boolean setColorList = getSetSummaryColorList();
            final boolean setColor = getSetSummaryColor();
            if (!TextUtils.isEmpty(summary)) {
                summaryView.setText(summary);
                if(setColor){
                    summaryView.setTextColor(summaryColor);
                }
                if(setColorList && (summaryColorList != null)){
                    summaryView.setTextColor(summaryColorList);
                }
                summaryView.setVisibility(View.VISIBLE);
                empty = false;
                auroraheight = mMinHeightWithSummary;
            } else {
                summaryView.setVisibility(View.GONE);
            }
             RelativeLayout layout = (RelativeLayout)view.findViewById(R.id.aurora_preferece_title_layout);
             setPaddingForPreference(layout);
        }
        // Aurora <Luofu> <2013-10-25> modify for auroraArrowText begin
        final TextView statusView = (TextView) view.findViewById(
                R.id.aurorastatus);
        if (statusView != null) {
            final CharSequence status = getStatus();
            final int statusColor = getStatusColor();
            final ColorStateList statusColorList = getStatusColorList();
            final boolean setColorList = getSetStatusColorList();
            final boolean setColor = getSetStatusColor();
            if (!TextUtils.isEmpty(status)) {
                statusView.setText(status);
                if(setColor){
                    statusView.setTextColor(statusColor);
                }
                if(setColorList && (statusColorList != null)){
                    statusView.setTextColor(statusColorList);
                }
        ...
        // final TextView commonStatusView = (TextView) view.findViewById(
        // R.id.aurorastatus1);
        // if (commonStatusView != null) {
        // Log.e("luofu", getStatus().toString());
        // final CharSequence status = getStatus();
        // if (!TextUtils.isEmpty(status)) {
        // commonStatusView.setText(status);
        // commonStatusView.setVisibility(View.VISIBLE);
        // } else {
        // commonStatusView.setVisibility(View.GONE);
        // }
        // }

        final ImageView arrow = (ImageView) view
                .findViewById(R.id.aurora_arrow_inner);
        if (arrow != null) {
           
            if (auroraGetShowArrow()) {
                
                arrow.setVisibility(View.VISIBLE);
            }
        }
        // Aurora <Luofu> <2013-10-25> modify for auroraArrowText end
...
//                    imageView.setLayoutParams(new LinearLayout.LayoutParams(width, height));
        ...
        // added by flash 2013.10.21
        // auroraShowArrowText();
        // end

        // added by flash 2013.10.11, all empty is added by flash,set min height
        auroraSetPreferenceItemHeight(view, auroraheight);
        if (empty)
        {
            auroraSetPreferenceItemHeight(view, mEmptyHeight);
        }
    }

    // added by flash 2013.10.21,show default textView start
    private TextView auroraArrowTextView;
    private String auroraArrowText;

    // added by flash 2013.10.21,show default textView end

    /*
     * adap preference item height params: v: which preference element height :
     * set to this height minHeight : true means most min height ,otherwise the
     * height forever
     */
    private void auroraSetPreferenceItemHeight(View v, int height)
    {
        ViewGroup.LayoutParams params = v.getLayoutParams();
        // params.height = height*3;
        // if(minHeight)
        v.setMinimumHeight(DensityUtil.dip2px(mContext, height));
        // else
     ...
     * objects on the same level. If this is not specified, the default behavior
     * is to sort alphabetically. The
    ...
    /**
     * Sets the title for this Preference with a CharSequence. This title will
     * be placed into the ID {@link R.id#title} within the View created by
    ...
    /**
     * Returns the title resource ID of this Preference. If the title did not
     * come from a resource, 0 is returned.
     * 
    ...
    /**
     * Sets the icon for this Preference with a Drawable. This icon will be
     * placed into the ID {@link R.id#icon} within the View created by
     * {@link #onCreateView(ViewGroup)}.
     * 
    ...
    // Aurora <Luofu> <2013-10-25> modify for status arrow begin
    public CharSequence getStatus() {
        return mStatus;
    }

    public void auroraSetArrowText(CharSequence status, boolean showArrow) {
    ...
    public void auroraSetArrowText(CharSequence status) {
        if (status == null && mStatus != null || status != null && !status.equals(mStatus)) {
            mStatus = status;
            notifyChanged();
        }
    ...
    private boolean auroraGetShowArrow() {
        return mShowArrow;
    }

    // Aurora <Luofu> <2013-10-25> modify for status arrow end
    /**
     ...
     * 
     * @param shouldDisableView Set true if this preference should disable its
     *            view when the preference is disabled.
     */
    ...
    /**
     * Checks whether this Preference should disable its view when it's action
     * is disabled.
     * 
     * @see #setShouldDisableView(boolean)
    ...
    /**
     * Returns a unique ID for this Preference. This ID should be unique across
     * all Preference objects in a hierarchy.
     * 
     ...
     * the {@link SharedPreferences}. However, the overridden method should call
     * {@link #callChangeListener(Object)} to make sure the client wants to
    ...
    /**
     * Checks whether the key is present, and if it isn't throws an exception.
     * This should be called by subclasses that store preferences in the
    ...
    /**
     * Checks whether this Preference is persistent. If it is, it stores its
     * value(s) into the persistent {@link SharedPreferences} storage.
     * 
    ...
    /**
     * Checks whether, at the given time this method is called, this Preference
     * should store/restore its value(s) into the {@link SharedPreferences}.
     * This, at minimum, checks whether this Preference is persistent and it
     * currently has a key. Before you save/restore from the
     * {@link SharedPreferences}, check this first.
     * 
    ...
    /**
     * Sets whether this Preference is persistent. When persistent, it stores
     * its value(s) into the persistent {@link SharedPreferences} storage.
     * 
    ...
    protected boolean callChangeListener(Object newValue) {
        return mOnChangeListener == null ? true : mOnChangeListener.onPreferenceChange(this,
                newValue);
    }
     ...
     */
    void performClick(AuroraPreferenceScreen preferenceScreen) {
        ...
        AuroraPreferenceManager preferenceManager = getPreferenceManager();
        if (preferenceManager != null) {
            AuroraPreferenceManager.OnPreferenceTreeClickListener listener = preferenceManager
    ...
    /**
     * Allows a Preference to intercept key events without having focus. For
     * example, SeekBarPreference uses this to intercept +/- to adjust the
     * progress.
     * 
    ...
    /**
     * Returns the {@link android.content.Context} of this Preference. Each
     * Preference in a Preference hierarchy can be from different Context (for
     * example, if multiple activities provide preferences into a single
     * {@link PreferenceActivity}). This Context will be used to save the
     * Preference values.
     * 
     ...
     * {@link #getSharedPreferences()}. If {@link #shouldCommit()} returns true,
     * it is this Preference's responsibility to commit.
     * <p>
    ...
    /**
     * Returns whether the {@link Preference} should commit its saved value(s)
     * in {@link #getEditor()}. This may return false in situations where batch
     ...
     * @return 0 if the same; less than 0 if this Preference sorts ahead of
     *         <var>another</var>; greater than 0 if this Preference sorts after
     *         <var>another</var>.
     */
    public int compareTo(AuroraPreference another) {
        if (mOrder != DEFAULT_ORDER
            ...
            // Do name comparison
            return AuroraCharSequences.compareToIgnoreCase(mTitle, another.mTitle);
        }
    }
    ...
    /**
     * Should be called when a Preference has been added/removed from this
     * group, or the ordering should be re-evaluated.
     */
    ...
    /**
     * Gets the {@link PreferenceManager} that manages this Preference object's
     * tree.
     * 
     ...
     * {@link PreferenceActivity}. This can also be called when this Preference
     * has been attached to a group that was already attached to the
    ...
    private void registerDependency() {
        ...
        AuroraPreference preference = findPreferenceInHierarchy(mDependencyKey);
        if (preference != null) {
    ...
    private void unregisterDependency() {
        if (mDependencyKey != null) {
            final AuroraPreference oldDependency = findPreferenceInHierarchy(mDependencyKey);
    ...
    /**
     * Finds a Preference in this hierarchy (the whole thing, even above/below
     * your {@link PreferenceScreen} screen break) with the given key.
     * <p>
     ...
     */
    protected AuroraPreference findPreferenceInHierarchy(String key) {
     ...
     */
    private void registerDependent(AuroraPreference dependent) {
        if (mDependents == null) {
            mDependents = new ArrayList<AuroraPreference>();
        }
     ...
     *         into a single statement.
     */
    private void unregisterDependent(AuroraPreference dependent) {
        if (mDependents != null) {
     ...
     * dependency.
     * 
     * @param disableDependents Whether this Preference should disable its
     *            dependents.
     */
    public void notifyDependencyChange(boolean disableDependents) {
        final List<AuroraPreference> dependents = mDependents;
     ...
     */
    public void onDependencyChanged(AuroraPreference dependency, boolean disableDependent) {
     ...
     * value to defaultValue that is given (and possibly store to
     * SharedPreferences if {@link #shouldPersist()} is true).
     * <p>
     * This may not always be called. One example is if it should not persist
     * but there is no default value given.
     * 
     * @param restorePersistedValue True to restore the persisted value; false
     *            to use the given <var>defaultValue</var>.
     ...
     * {@link PreferenceManager}, put in the string, and check if we should
     ...
     * user input.
     * <p>
     * If overridding and calling through to the superclass, make sure to
     * prepend your additions with a space.
     * 
     ...
     * 
     * @param container The Bundle in which to save the instance of this
     *            Preference.
    ...
    /**
     * Called by {@link #saveHierarchyState} to store the instance for this
     * Preference and its children. May be overridden to modify how the save
     * happens for children. For example, some Preference objects may want to
     * not store an instance for their children.
     * 
     * @param container The Bundle in which to save the instance of this
     *            Preference.
     ...
     *         Preference, or null if there is nothing interesting to save. The
    ...
    /**
     * Called by {@link #restoreHierarchyState} to retrieve the saved state for
     * this Preference and its children. May be overridden to modify how
     * restoring happens to the children of a Preference. For example, some
     * Preference objects may not want to save state for their children.
     * 
     * @param container The Bundle that holds the previously saved state.
     * @see #restoreHierarchyState
     * @see #onRestoreInstanceState
    ...
    private void AnalyzeNativePreferenceAttributeSet(Context context, AttributeSet attrs,
            int defStyle) {
        ...
        for (i = 0; i < attrs.getAttributeCount(); i++) {
            ...
            switch (id) {
                case android.R.attr.icon:
                    mIconResId = attrs.getAttributeIntValue(i, 0);
                    break;

                case android.R.attr.key:
                    mKey = NativePreferenceManager.getAttributeStringValue(context, attrs, i);
                    break;

                case android.R.attr.title:
                    mTitle = NativePreferenceManager.getAttributeStringValue(context, attrs, i);
                    break;
                ...
                case android.R.attr.order:
                    mOrder = attrs.getAttributeIntValue(i, mOrder);
                    break;

                case android.R.attr.fragment:
                    mFragment = attrs.getAttributeValue(i);
                    break;

                case android.R.attr.layout:
                    mLayoutResId = attrs.getAttributeIntValue(i, mLayoutResId);
                    break;

                case android.R.attr.widgetLayout:
                    mWidgetLayoutResId = attrs.getAttributeIntValue(i, mWidgetLayoutResId);
                    break;

                case android.R.attr.enabled:
                    mEnabled = attrs.getAttributeBooleanValue(i, true);
                    break;

                case android.R.attr.selectable:
                    mSelectable = attrs.getAttributeBooleanValue(i, true);
                    break;

                case android.R.attr.persistent:
                    mPersistent = attrs.getAttributeBooleanValue(i, mPersistent);
                    break;

                case android.R.attr.dependency:
                    mDependencyKey = NativePreferenceManager.getAttributeStringValue(context,
                            attrs, i);
                    break;
                ...
                case android.R.attr.shouldDisableView:
                    mShouldDisableView = attrs.getAttributeBooleanValue(i, mShouldDisableView);
                    break;
            }
        }
    }

    private void AnalyzeAuroraPreferenceAttributeSet(Context context, AttributeSet attrs,
            int defStyle) {
        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.AuroraPreference, defStyle, 0);
        for (int i = a.getIndexCount(); i >= 0; i--) {
            int attr = a.getIndex(i);
            if (attr == R.styleable.AuroraPreference_auroraicon) {
                mIconResId = a.getResourceId(attr, 0);

            } else if (attr == R.styleable.AuroraPreference_aurorakey) {
                mKey = a.getString(attr);

            } else if (attr == R.styleable.AuroraPreference_auroratitle) {
                mTitleRes = a.getResourceId(attr, 0);
                mTitle = a.getString(attr);

            } else if (attr == R.styleable.AuroraPreference_aurorasummary) {
                mSummary = a.getString(attr);

            } else if (attr == R.styleable.AuroraPreference_auroraorder) {
                mOrder = a.getInt(attr, mOrder);

            } else if (attr == R.styleable.AuroraPreference_aurorafragment) {
                mFragment = a.getString(attr);

            } else if (attr == R.styleable.AuroraPreference_auroralayout) {
                mLayoutResId = a.getResourceId(attr, mLayoutResId);

            } else if (attr == R.styleable.AuroraPreference_aurorawidgetLayout) {
                mWidgetLayoutResId = a.getResourceId(attr, mWidgetLayoutResId);


                // add by <flash>2013.10.16 start
            } else if (attr == R.styleable.AuroraPreference_aurorabottomwidgetLayout) {
                auroraBottomLayoutResId = a.getResourceId(attr, auroraBottomLayoutResId);

                // end
            } else if (attr == R.styleable.AuroraPreference_auroraenabled) {
                mEnabled = a.getBoolean(attr, true);

            } else if (attr == R.styleable.AuroraPreference_auroraselectable) {
                mSelectable = a.getBoolean(attr, true);

            } else if (attr == R.styleable.AuroraPreference_aurorapersistent) {
                mPersistent = a.getBoolean(attr, mPersistent);

            } else if (attr == R.styleable.AuroraPreference_auroradependency) {
                mDependencyKey = a.getString(attr);

            } else if (attr == R.styleable.AuroraPreference_auroradefaultValue) {
                mDefaultValue = onGetDefaultValue(a, attr);

            } else if (attr == R.styleable.AuroraPreference_aurorashouldDisableView) {

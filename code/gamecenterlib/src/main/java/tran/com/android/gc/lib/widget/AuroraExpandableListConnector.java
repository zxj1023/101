 ...
 * expects.
 */
class AuroraExpandableListConnector extends BaseAdapter implements Filterable {
    /**
     ...
     */
    public AuroraExpandableListConnector(ExpandableListAdapter expandableListAdapter) {
    ...
    PositionMetadata getUnflattenedPos(final int flPos) {
        ...
        if (numExpGroups == 0) {
             ...
             * same as its flPos
             */
            return PositionMetadata.obtain(flPos, AuroraExpandableListPosition.GROUP, flPos,
                    -1, null, 0);
        }
        ...
        while (leftExpGroupIndex <= rightExpGroupIndex) {
            ...
            if (flPos > midExpGm.lastChildFlPos) {
                 ...
                 */
                return PositionMetadata.obtain(flPos, AuroraExpandableListPosition.GROUP,
                ...
                return PositionMetadata.obtain(flPos, AuroraExpandableListPosition.CHILD,
        ...
        return PositionMetadata.obtain(flPos, AuroraExpandableListPosition.GROUP, groupPos, -1,
                null, insertPosition);
    }
     ...
     */
    PositionMetadata getFlattenedPos(final AuroraExpandableListPosition pos) {
        final ArrayList<GroupMetadata> egml = mExpGroupMetadataList;
        final int numExpGroups = egml.size();
        ...
        while (leftExpGroupIndex <= rightExpGroupIndex) {
            ...
            if (pos.groupPos > midExpGm.gPos) {
                ...
                if (pos.type == AuroraExpandableListPosition.GROUP) {
                ...
                } else if (pos.type == AuroraExpandableListPosition.CHILD) {
    ...
    public boolean isEnabled(int flatListPos) {
        ...
        final AuroraExpandableListPosition pos = metadata.position;
        
        boolean retValue;
        if (pos.type == AuroraExpandableListPosition.CHILD) {
    ...
    public Object getItem(int flatListPos) {
        ...
        } else if (posMetadata.position.type == AuroraExpandableListPosition.CHILD) {
    ...
    public View getView(int flatListPos, View convertView, ViewGroup parent) {
        ...
        if (posMetadata.position.type == AuroraExpandableListPosition.GROUP) {
            ...
            /*if (posMetadata.isExpanded() && 
            		mExpandableListAdapter.getChildrenCount(posMetadata.position.groupPos) > 0) {
            	retValue.setBackgroundColor(R.drawable.aurora_transparent);
            } else {
            	retValue.setBackgroundResource(R.drawable.aurora_expandlist_group_bg_light);
            }*/
        } else if (posMetadata.position.type == AuroraExpandableListPosition.CHILD) {
            final boolean isLastChild = posMetadata.groupMetadata.lastChildFlPos == flatListPos;
            final boolean isFirstChild = (posMetadata.groupMetadata.flPos + 1) == flatListPos;
            ...
            /*if (mIsExpandGroup) {
                Animator anim = createExpandAnimaForView(retValue);
                Animator alphAnim = createAlphaAnimaForView(retValue);
                AnimatorSet animatorSet = new AnimatorSet();
                animatorSet.playTogether(anim, alphAnim);
                animatorSet.start();
                Animation animation = new ExpandGroupAnimation(retValue);
                retValue.setAnimation(animation);
            }*/
        } else {
            // TODO: clean exit
            throw new RuntimeException("Flat list position is of unknown type");
    ...
    static public class PositionMetadata {
        ...
        public AuroraExpandableListPosition position;
        ...
        static PositionMetadata obtain(int flatListPos, int type, int groupPos,
            ...
            pm.position = AuroraExpandableListPosition.obtain(type, groupPos, childPos, flatListPos);
     ...
     * Constructs the connector
     */
    public AuroraExpandableListConnector(Context context, ExpandableListAdapter expandableListAdapter) {
        mExpGroupMetadataList = new ArrayList<GroupMetadata>();
	...
	private Animator createExpandAnimaForView(final View view) {
		final ViewGroup.LayoutParams lp = view.getLayoutParams();
		final int originalHeight = view.getHeight() != 0 ? view.getHeight() : 1;
    	Log.v("GnExpandListView", "createExpandAnimaForView originalHeight" + originalHeight);
    	
    	final ValueAnimator animator = ValueAnimator.ofInt(originalHeight, 128);
		...
		animator.addListener(new AnimatorListener() {
			...
			public void onAnimationEnd(Animator arg0) {
		    	Log.v("GnExpandListView", "createExpandAnimaForView onAnimationEnd");
				lp.height = 128;
		...
		animator.addUpdateListener(new AnimatorUpdateListener() {
			...
			public void onAnimationUpdate(ValueAnimator valueAnimator) {
				...
				Log.v("GnExpandListView", "createExpandAnimaForView onAnimationUpdate lp.height = " + lp.height);
				view.setLayoutParams(lp);
			}
		});
		return animator;
	...
	void setExpandAnimFlg(boolean anim) {
		...
		Log.v("GnExpandListView", "mIsExpandGroup = " + mIsExpandGroup);
	}
    ...
    private void setChildViewBackground(View view, boolean isLastChild,
			boolean isFirstChild) {
    	if (isFirstChild && isLastChild) {
    		view.setBackgroundResource(mChildViewBackgroudRes[FRAME_LIST_BACKGROUND_FULL]);
    	} else if (isFirstChild) {
        	view.setBackgroundResource(mChildViewBackgroudRes[FRAME_LIST_BACKGROUND_TOP]);
        } else if (isLastChild) {
        	view.setBackgroundResource(mChildViewBackgroudRes[FRAME_LIST_BACKGROUND_BOTTOM]);
        } else {
        	view.setBackgroundResource(mChildViewBackgroudRes[FRAME_LIST_BACKGROUND_MIDDLE]);
        }
	}
    
    private void getFrameListBackground(Context context) {
    	mChildViewBackgroudRes = new int[FRAME_LIST_BACKGROUND_TOTAL];
    	mChildViewBackgroudRes[FRAME_LIST_BACKGROUND_NULL] = 0;
        TypedValue outValue = new TypedValue();
        
        context.getTheme().resolveAttribute(R.attr.auroraframeListBackground,
                outValue, true);
        mChildViewBackgroudRes[FRAME_LIST_BACKGROUND_FULL] = outValue.resourceId;
        context.getTheme().resolveAttribute(R.attr.auroraframeListTopBackground,
                outValue, true);
        mChildViewBackgroudRes[FRAME_LIST_BACKGROUND_TOP] = outValue.resourceId;
        context.getTheme().resolveAttribute(R.attr.auroraframeListMiddleBackground,
                outValue, true);
        mChildViewBackgroudRes[FRAME_LIST_BACKGROUND_MIDDLE] = outValue.resourceId;
        context.getTheme().resolveAttribute(R.attr.auroraframeListBottomBackground,
                outValue, true);
        mChildViewBackgroudRes[FRAME_LIST_BACKGROUND_BOTTOM] = outValue.resourceId;
    }
    
	private class ExpandGroupAnimation extends Animation {
		private float mStart;
		private float mEnd;
		private ViewGroup.LayoutParams lp;
		private View mView;
		private float mStartHeight = 0;
		private float mDeltaHeight = 128;
		
		public ExpandGroupAnimation(View view) {
			mStart = 0.0f;
			mEnd = 1.0f;
			mView = view;
		...
		protected void applyTransformation(float interpolatedTime, Transformation t) {
	        final float alpha = mStart;
	        t.setAlpha(alpha + ((mEnd - alpha) * interpolatedTime));

...
public class AuroraActionBar extends LinearLayout {
	...
	public AuroraActionBar(Context context, AttributeSet attrs, int defStyle) {
		...
		// 获取各个属性的值
		mTitle = "transsionChanged"; //a.getString( R.styleable.AuroraActionBar_title);
		mMaxItemsCount = a.getInt(R.styleable.AuroraActionBar_maxItems, 3);
		int layoutID;
		int type = a.getInteger( R.styleable.AuroraActionBar_type, -1);
	...
	private void bindViewsEvent( ) {
		...
		case Custom:
			...
			if(!TextUtils.isEmpty(mHomeTextView.getText())){
     ...
     private static Typeface auroraCreateTitleFont( ) {
    	try {
			if ( Locale.getDefault().getLanguage().equals("zh") ) {
				auroraTitleFace = Typeface.createFromFile(AuroraUtil.ACTION_BAR_TITLE_FONT);
			} else {
				auroraTitleFace = Typeface.createFromFile(AuroraUtil.ACTION_BAR_TITLE_FONT_FORENGLISH);
			}
...
//	            titleWidth = (int)paint.measureText(title.toString());
	...
	private OnTouchListener backButtonChangeListener_custom = new OnTouchListener() {
		...
		public boolean onTouch(View v, MotionEvent event) {
			...
			if (event.getAction() == MotionEvent.ACTION_DOWN) {
				AuroraLog.e("222222", "--backButtonChangeListener_custom--ACTION_DOWN--");
				mHomeButton.onTouchEvent(event);
				if ( mHomeTextView != null ) {
					mHomeTextView.onTouchEvent(event);
				}
			...
			if (event.getAction() == MotionEvent.ACTION_UP) {
				AuroraLog.e("222222", "--backButtonChangeListener_custom--ACTION_UP--");
				mHomeButton.setPressed(false);
				if ( mHomeTextView != null ) {
					mHomeTextView.setPressed(false);
				}
...
//		showActionBottomeBarMenu();
	...
	private void auroraPlayEditModeAnim()
	{
	 ...
	 private void auroraPlayExitEditModeAnim()
	 {
		 ...
		 ObjectAnimator alphaIn = ObjectAnimator.ofFloat(auroraCurActionBarContent, "Alpha", 1.0f,1.0f);

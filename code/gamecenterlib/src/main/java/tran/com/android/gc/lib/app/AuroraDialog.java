 ...
 * {@link Activity#onPrepareDialog(int, AuroraDialog)},
 ...
 * </div>
 */
public class AuroraDialog extends Dialog implements DialogInterface,
		Window.Callback, KeyEvent.Callback, OnCreateContextMenuListener,
		AnimationListener {

	public static final String ACTION_FROM_MENU = "com.aurora.DIALOG_ACTION_FROM_MENU";
	public static final String ACTION_FROM_INTERNAL = "com.aurora.DIALOG_FROM_INTENRAL";
	...
	protected boolean mAuroraStyle;

	protected boolean mHasView;
	private boolean mHideView = false;

	private boolean mIsInvokedShow = false;
	private boolean mFromMenu = false;
	private boolean mNeedShow = false;
	private int mShowDelay = 350;
	private int mAnimationDuration = 200;
	private float mRotationDegree = 10f;
	private float mDialogWidth = 313f;

	private float mDialogDimMount = 0.65f;
	...
	protected AuroraDialog(Context context, int theme,
			boolean createContextThemeWrapper) {
		super(context, theme);
		if (createContextThemeWrapper) {
			if (theme == 0) {
				// TypedValue outValue = new TypedValue();
				// context.getTheme().resolveAttribute(R.attr.auroraDialogTheme,
				// outValue, true);
				// theme = outValue.resourceId;
				theme = R.style.AuroraAlertDialogTheme;
			}
	 ...
	 */
	@Deprecated
	protected AuroraDialog(Context context, boolean cancelable,
	...
	private void applyEnterAnimation(float start, float end, View view) {
		final float centerX = getRotateXPosition();// view.getWidth() / 2.0f;
		final float centerY = 0;

		final AuroraDialogEnterAnimation enter = new AuroraDialogEnterAnimation(
				view, start, end, centerX, centerY, 0.0f, false);
		enter.setDuration(mAnimationDuration);
	...
	private void showProxy() {
		if (mIsInvokedShow && !Constants.SHOW_DIALOAG_FROM_MENU) {
			mHideView = false;
			showInternal(mHideView);

		} else if (mIsInvokedShow && Constants.SHOW_DIALOAG_FROM_MENU) {
			mHideView = true;
			showInternal(mHideView);
			mShowHandler.postDelayed(mShowAction, mShowDelay);
			Constants.SHOW_DIALOAG_FROM_MENU = false;
		}

	}

	/**
	 * if you want to not only show a dialog but also play it's animation with
	 * 3d, you should call this method before show();
	 */
	public void showFromPopupMenu() {
		Constants.SHOW_DIALOAG_FROM_MENU = true;
	}

	public static void showFromPopupWindow() {
		Constants.SHOW_DIALOAG_FROM_MENU = true;
	}
	...
	public void setCanceledOnTouchOutside(boolean cancel) {
		// TODO Auto-generated method stub
		mCancelOutSide = cancel;
		// super.setCanceledOnTouchOutside(cancel);
	}

	/**
	 * 该方法主要是为了处理在弹窗外面区域的点击事件
	 * 
	 * @param parent
	 */
	protected void doTouchOutsideEvents(final View content, final View decor) {
		AuroraLog.e("touch", "isOutOfBoundsOnWindow( event, content):");
		if (content == null) {
			return;
		}
	//	if (mCancelOutSide) {
		//	super.setCanceledOnTouchOutside(false);
			decor.setOnTouchListener(new OnTouchListener() {
				...
				public boolean onTouch(View view, MotionEvent event) {
					...
					AuroraLog.e("touch",
							"isOutOfBoundsOnWindow( event, content):"
									+ isOutOfBoundsOnWindow(event, content));
					if (action == MotionEvent.ACTION_DOWN) {
						AuroraLog.e("touch", "down*******");
					} else if (action == MotionEvent.ACTION_UP) {
						AuroraLog.e("touch", "ACTION_UP-----------------");
						if (isOutOfBoundsOnWindow(event, content)) {
							dismiss();
	...
	private boolean isOutOfBoundsOnWindow(MotionEvent event, View target) {
		final int x = (int) event.getRawX();
		final int y = (int) event.getRawY();
		final int slop = ViewConfiguration.get(mContext)
				.getScaledWindowTouchSlop();
		int[] displaySize = DensityUtil.getDisplayHeight(mContext);

		int screenWidth = displaySize[1];
		int screenHeight = displaySize[0];
		return x > (screenWidth - target.getWidth()) / 2 + target.getWidth()
				|| y > (screenHeight - target.getHeight()) / 2
						+ target.getHeight()
				|| x < (screenWidth - target.getWidth()) / 2
				|| y < (screenHeight - target.getHeight()) / 2;

	}

	private void applyAnimation() {
		View dialogContentView = mDecor
				.findViewById(R.id.aurora_parentPanel);
		// if (dialogContentView != null) {
		// applyEnterAnimation(mRotationDegree, 0, dialogContentView);
		//
		// }
		View datePickerContent = mDecor
				.findViewById(R.id.aurora_date_picker_layout);
		View timePickerContent = mDecor
				.findViewById(R.id.aurora_time_picker_layout);

		if (datePickerContent != null) {
			applyEnterAnimation(mRotationDegree, 0, datePickerContent);
		}

		if (timePickerContent != null) {
			applyEnterAnimation(mRotationDegree, 0, timePickerContent);
		}
	}

	private float getRotateXPosition() {
		return DensityUtil.dip2px(mContext, mDialogWidth) / 2;
	}

	protected void auroraStyle(boolean flag) {
		mAuroraStyle = flag;
	}
	...
	void dismissDialog() {
		...
		if (mWindow.isDestroyed()) {
			Log.e(TAG,
					"Tried to dismissDialog() but the Dialog's window was already destroyed!");
			return;
		}
	...
	/**
	 * Saves the state of the dialog into a bundle. The default implementation
	 * saves the state of its view hierarchy, so you'll likely want to call
	 * through to super if you override this to save additional state.
	 * 
	...
	/**
	 * Restore the state of the dialog from a previously saved bundle. The
	 * default implementation restores the state of the dialog's view hierarchy
	 * that was saved in the default implementation of
	...
	public void onRestoreInstanceState(Bundle savedInstanceState) {
		final Bundle dialogHierarchyState = savedInstanceState
				.getBundle(DIALOG_HIERARCHY_TAG);
	...
	public boolean dispatchKeyEvent(KeyEvent event) {
		...
		return event.dispatch(this,
				mDecor != null ? mDecor.getKeyDispatcherState() : null, this);
	}
	...
	/**
	 * {@inheritDoc} Note that if you override this method you should always
	 * call through to the superclass implementation by calling
	...
	private ComponentName getAssociatedActivity() {
		...
		while (activity == null && context != null) {
			if (context instanceof Activity) {
				...
				context = (context instanceof ContextWrapper) ? ((ContextWrapper) context)
						.getBaseContext() : // unwrap
											// one
											// level
						null; // done
			}
	 ...
	 * <p>
	 * This will only be invoked when the dialog is canceled. Cancel events
	 * alone will not capture all ways that the dialog might be dismissed. If
	 * the creator needs to know when a dialog is dismissed in general, use
	...
	public void setOnDismissListener(final OnDismissListener listener) {
		...
		if (listener != null) {
			mDismissMessage = mListenersHandler
					.obtainMessage(DISMISS, listener);
		} else {
	...
	/** @hide */
	public boolean takeCancelAndDismissListeners(String msg,
			final OnCancelListener cancel, final OnDismissListener dismiss) {
		if (mCancelAndDismissTaken != null) {
	...
	private static final class ListenersHandler extends Handler {
		...
		public ListenersHandler(AuroraDialog dialog) {

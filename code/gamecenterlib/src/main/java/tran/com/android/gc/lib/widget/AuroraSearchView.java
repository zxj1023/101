...
import tran.com.android.gc.lib.utils.HomeKeyWatcher;
import tran.com.android.gc.lib.utils.HomeKeyWatcher.OnHomePressedListener;
    ...
    private Runnable mReleaseCursorRunnable = new Runnable() {
        public void run() {
            if (mSuggestionsAdapter != null
                    && mSuggestionsAdapter instanceof AuroraSuggestionsAdapter) {
                // / M: When release cursor, just close it.
                ((AuroraSuggestionsAdapter) mSuggestionsAdapter).close();
            }
        }
    };
...
//        mSearchHintIcon = (ImageView) findViewById(R.id.aurora_search_mag_icon);
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, android.R.styleable.SearchView, 0, 0);
        // setIconifiedByDefault(a.getBoolean(android.R.styleable.SearchView_iconifiedByDefault,
        // true));
        int maxWidth = a.getDimensionPixelSize(android.R.styleable.SearchView_maxWidth, -1);
        if (maxWidth != -1) {
            setMaxWidth(maxWidth);
        }
        CharSequence queryHint = a.getText(android.R.styleable.SearchView_queryHint);
        if (!TextUtils.isEmpty(queryHint)) {
            setQueryHint(queryHint);
        }
        int imeOptions = a.getInt(android.R.styleable.SearchView_imeOptions, -1);
        if (imeOptions != -1) {
            setImeOptions(imeOptions);
        }
        int inputType = a.getInt(android.R.styleable.SearchView_inputType, -1);
        if (inputType != -1) {
        ...
        background = a.getDrawable(android.R.styleable.View_background);
        a.recycle();

        int maxLength = 0;
        ColorStateList textColor = null;
        a = context.obtainStyledAttributes(attrs, android.R.styleable.TextView, 0, 0);
        maxLength = a.getInteger(android.R.styleable.TextView_maxLength, 0);
        textColor = a.getColorStateList(android.R.styleable.TextView_textColor);
        a.recycle();
...
//        }
    ...
    public void setTextSize(float size){
    	  if (mQueryTextView != null) {
              mQueryTextView.setTextSize(size);
          }
    }
    ...
    private void updatePadding() {
        ...
        // getResources().getDimensionPixelSize(R.dimen.aurora_search_view_hint_icon_padding_left)
        // +getResources().getDimensionPixelSize(R.dimen.aurora_search_view_hint_text_padding_left);
        // //+glassWidth;
    ...
    private void updateCloseButton() {
        ...
        // }
//        mHintIcon.setVisibility(showClose ? GONE : VISIBLE);
        mCloseButton.setVisibility(showClose ? VISIBLE : GONE);
        splitLine.setVisibility(showClose ? VISIBLE : GONE);
    ...
    private static String getActionKeyMessage(Cursor c, SearchableInfo.ActionKeyInfo actionKey) {
        ...
        if (column != null) {
            result = AuroraSuggestionsAdapter.getColumnString(c, column);
        }

 ...
 */
public abstract class AuroraPreferenceGroup extends AuroraPreference implements AuroraGenericInflater.Parent<AuroraPreference> {
    /**
    ...
    public AuroraPreferenceGroup(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (NativePreferenceManager.getAnalyzeNativePreferenceXml() && attrs != null) {
            ...
            for (i=0; i<attrs.getAttributeCount(); i++) {
                ...
                switch (id) {
                    case android.R.attr.orderingFromXml:
                        mOrderingAsAdded = attrs.getAttributeBooleanValue(i, mOrderingAsAdded);
                        break;
                }
            }
        } else {
        // Gionee <zhangxx> <2013-08-14> modify for CR00857086 end
            TypedArray a = context.obtainStyledAttributes(attrs,
                    R.styleable.AuroraPreferenceGroup, defStyle, 0);
            mOrderingAsAdded = a.getBoolean(R.styleable.AuroraPreferenceGroup_auroraorderingFromXml,
                    mOrderingAsAdded);
            a.recycle();
        // Gionee <zhangxx> <2013-08-14> modify for CR00857086 begin
        }
    ...
    public AuroraPreferenceGroup(Context context, AttributeSet attrs) {
     ...
     */
    public void addItemFromInflater(AuroraPreference preference) {
     ...
     */
    public AuroraPreference getPreference(int index) {
    ...
    public boolean addPreference(AuroraPreference preference) {
        ...
        if (preference.getOrder() == AuroraPreference.DEFAULT_ORDER) {
            if (mOrderingAsAdded) {
            ...
            if (preference instanceof AuroraPreferenceGroup) {
                ...
                ((AuroraPreferenceGroup)preference).setOrderingAsAdded(mOrderingAsAdded);
            }
        }
    ...
    private boolean removePreferenceInt(AuroraPreference preference) {
        synchronized(this) {
    ...
    public void removeAll() {
        synchronized(this) {
            List<AuroraPreference> preferenceList = mPreferenceList;
    ...
    public AuroraPreference findPreference(CharSequence key) {
        ...
        for (int i = 0; i < preferenceCount; i++) {
            final AuroraPreference preference = getPreference(i);
            ...
            if (preference instanceof AuroraPreferenceGroup) {
                final AuroraPreference returnedPreference = ((AuroraPreferenceGroup)preference)
                        .findPreference(key);

...
import universalimageloader.core.assist.FailReason.FailType;
...
import universalimageloader.core.decode.ImageDecoder;
import universalimageloader.core.decode.ImageDecodingInfo;
import universalimageloader.core.download.ImageDownloader;
import universalimageloader.core.download.ImageDownloader.Scheme;
...
import tran.com.android.tapla.gamecenter.market.util.Blur;

/**
 * Presents load'n'display image task. Used to load image from Internet or file system, decode it to {@link Bitmap}, and
 * display it in {@link ImageAware} using {@link universalimageloader.core.DisplayBitmapTask}.
 *
 * @author Sergey Tarasevich (nostra13[at]gmail[dot]com)
 * @see ImageLoaderConfiguration
 * @see universalimageloader.core.ImageLoadingInfo
 * @since 1.3.1
 */
final class LoadAndDisplayImageTask implements Runnable, IoUtils.CopyListener {
	...
	private final universalimageloader.core.ImageLoadingInfo imageLoadingInfo;
	...
	final universalimageloader.core.DisplayImageOptions options;
	...
	//0- normal 1-blur 2- no display image
	private int type = 0;
	public LoadAndDisplayImageTask(universalimageloader.core.ImageLoaderEngine engine, universalimageloader.core.ImageLoadingInfo imageLoadingInfo, Handler handler, Context context, int type) {
		this.engine = engine;
	...
	public void run() {
		...
		try {
			...
			if (bmp == null || bmp.isRecycled()) {
				...
				if(type == 1)
				{
					/*Bitmap bit = BitmapUtil.cropBitmap(bmp, bmp.getWidth(), m_context.getResources().getDimensionPixelOffset(R.dimen.special_item_tip_text_height)*bmp.getHeight()/m_context.getResources().getDimensionPixelOffset(R.dimen.special_item_image_height), true);
					bit = Blur.fastblur(m_context, bit, 10);
					bmp = BitmapUtil.combineBitmap1(bmp, bit);*/
					
					bit  = Blur.fastblur(m_context, bmp, 10);
		...
		universalimageloader.core.DisplayBitmapTask displayBitmapTask = new universalimageloader.core.DisplayBitmapTask(bmp, imageLoadingInfo, engine, loadedFrom,bit);
	...
	private boolean resizeAndSaveImage(int maxWidth, int maxHeight) throws IOException {
		...
		if (targetFile != null && targetFile.exists()) {
			...
			universalimageloader.core.DisplayImageOptions specialOptions = new universalimageloader.core.DisplayImageOptions.Builder().cloneFrom(options)
	...
	static void runTask(Runnable r, boolean sync, Handler handler, universalimageloader.core.ImageLoaderEngine engine) {
		if (sync) {
			r.run();

...
import universalimageloader.cache.disc.impl.ext.LruDiscCache;
 ...
 * @see ImageLoader
 * @see universalimageloader.cache.memory.MemoryCache
 * @see universalimageloader.cache.disc.DiskCache
 * @see DisplayImageOptions
 * @see universalimageloader.core.download.ImageDownloader
 * @see universalimageloader.cache.disc.naming.FileNameGenerator
 * @since 1.0.0
 */
public final class ImageLoaderConfiguration {
	...
	final universalimageloader.core.process.BitmapProcessor processorForDiskCache;
	...
	final universalimageloader.core.assist.QueueProcessingType tasksProcessingType;

	final universalimageloader.cache.memory.MemoryCache memoryCache;
	final universalimageloader.cache.disc.DiskCache diskCache;
	final universalimageloader.core.download.ImageDownloader downloader;
	final universalimageloader.core.decode.ImageDecoder decoder;
	final DisplayImageOptions defaultDisplayImageOptions;

	final universalimageloader.core.download.ImageDownloader networkDeniedDownloader;
	final universalimageloader.core.download.ImageDownloader slowNetworkDownloader;
	...
	private ImageLoaderConfiguration(final Builder builder) {
		...
		universalimageloader.utils.L.writeDebugLogs(builder.writeLogs);
	}
	 ...
	 * <li>diskCache = {@link UnlimitedDiscCache}</li>
	 ...
	 * <li>tasksProcessingOrder = {@link universalimageloader.core.assist.QueueProcessingType#FIFO}</li>
	...
	universalimageloader.core.assist.ImageSize getMaxImageSize() {
		...
		return new universalimageloader.core.assist.ImageSize(width, height);
	}
	...
	public static class Builder {
		...
		/** {@value} */
		public static final universalimageloader.core.assist.QueueProcessingType DEFAULT_TASK_PROCESSING_TYPE = universalimageloader.core.assist.QueueProcessingType.FIFO;
		...
		private universalimageloader.core.process.BitmapProcessor processorForDiskCache = null;
		...
		private universalimageloader.core.assist.QueueProcessingType tasksProcessingType = DEFAULT_TASK_PROCESSING_TYPE;

		private int memoryCacheSize = 0;
		private long diskCacheSize = 0;
		private int diskCacheFileCount = 0;

		private universalimageloader.cache.memory.MemoryCache memoryCache = null;
		private universalimageloader.cache.disc.DiskCache diskCache = null;
		private universalimageloader.cache.disc.naming.FileNameGenerator diskCacheFileNameGenerator = null;
		private universalimageloader.core.download.ImageDownloader downloader = null;
		private universalimageloader.core.decode.ImageDecoder decoder;
		...
		public Builder discCacheExtraOptions(int maxImageWidthForDiskCache, int maxImageHeightForDiskCache,
				universalimageloader.core.process.BitmapProcessor processorForDiskCache) {
		 ...
		 * @param processorForDiskCache      null-ok; {@linkplain universalimageloader.core.process.BitmapProcessor Bitmap processor} which process images before saving them in disc cache
		 */
		 ...
		 * <li>{@link #tasksProcessingOrder(universalimageloader.core.assist.QueueProcessingType)}</li>
		 * </ul>
		 *
		...
		public Builder taskExecutor(Executor executor) {
			if (threadPoolSize != DEFAULT_THREAD_POOL_SIZE || threadPriority != DEFAULT_THREAD_PRIORITY || tasksProcessingType != DEFAULT_TASK_PROCESSING_TYPE) {
				universalimageloader.utils.L.w(WARNING_OVERLAP_EXECUTOR);
			}
		 ...
		 * Default value - {@link universalimageloader.core.assist.QueueProcessingType#FIFO}
		 */
		public Builder tasksProcessingOrder(universalimageloader.core.assist.QueueProcessingType tasksProcessingType) {
			if (taskExecutor != null || taskExecutorForCachedImages != null) {
				universalimageloader.utils.L.w(WARNING_OVERLAP_EXECUTOR);
			}
		 ...
		 * memory cache. You can use {@link #memoryCache(universalimageloader.cache.memory.MemoryCache)} method to set your own implementation of
		 * {@link universalimageloader.cache.memory.MemoryCache}.
		 */
		public Builder memoryCacheSize(int memoryCacheSize) {
			if (memoryCacheSize <= 0) throw new IllegalArgumentException("memoryCacheSize must be a positive number");

			if (memoryCache != null) {
				universalimageloader.utils.L.w(WARNING_OVERLAP_MEMORY_CACHE);
			}
		 ...
		 * </ul>
		 */
		public Builder memoryCache(universalimageloader.cache.memory.MemoryCache memoryCache) {
		 ...
		 * <b>NOTE:</b> If you use this method then
		 * {@link LruDiscCache LruDiscCache}
		 * will be used as disk cache. You can use {@link #diskCache(universalimageloader.cache.disc.DiskCache)} method for introduction your own
		 * implementation of {@link universalimageloader.cache.disc.DiskCache}
		 */
		public Builder diskCacheSize(int maxCacheSize) {
			...
			if (diskCache != null) {
				universalimageloader.utils.L.w(WARNING_OVERLAP_DISK_CACHE_PARAMS);
			}
		...
		@Deprecated
		public Builder discCacheFileNameGenerator(universalimageloader.cache.disc.naming.FileNameGenerator fileNameGenerator) {
		 ...
		 */
		public Builder diskCacheFileNameGenerator(universalimageloader.cache.disc.naming.FileNameGenerator fileNameGenerator) {
			if (diskCache != null) {
				universalimageloader.utils.L.w(WARNING_OVERLAP_DISK_CACHE_NAME_GENERATOR);
			}
		...
		@Deprecated
		public Builder discCache(universalimageloader.cache.disc.DiskCache diskCache) {
			return diskCache(diskCache);
		}

		/**
		 * Sets disk cache for images.<br />
		 * Default value - {@link UnlimitedDiscCache
		 ...
		 * <li>{@link #diskCacheFileNameGenerator(universalimageloader.cache.disc.naming.FileNameGenerator)}</li>
		 * </ul>
		 */
		public Builder diskCache(universalimageloader.cache.disc.DiskCache diskCache) {
		 ...
		 */
		public Builder imageDownloader(universalimageloader.core.download.ImageDownloader imageDownloader) {
		 ...
		 */
		public Builder imageDecoder(universalimageloader.core.decode.ImageDecoder imageDecoder) {
		...
		private void initEmptyFieldsWithDefaultValues() {
			...
			if (denyCacheImageMultipleSizesInMemory) {
				memoryCache = new universalimageloader.cache.memory.impl.FuzzyKeyMemoryCache(memoryCache, universalimageloader.utils.MemoryCacheUtils.createFuzzyKeyComparator());
			}
			if (downloader == null) {
	 ...
	 * @since 1.8.0
	 */
	private static class NetworkDeniedImageDownloader implements universalimageloader.core.download.ImageDownloader {

		private final universalimageloader.core.download.ImageDownloader wrappedDownloader;

		public NetworkDeniedImageDownloader(universalimageloader.core.download.ImageDownloader wrappedDownloader) {
	 ...
	 * @since 1.8.1
	 */
	private static class SlowNetworkImageDownloader implements universalimageloader.core.download.ImageDownloader {

		private final universalimageloader.core.download.ImageDownloader wrappedDownloader;

		public SlowNetworkImageDownloader(universalimageloader.core.download.ImageDownloader wrappedDownloader) {
			this.wrappedDownloader = wrappedDownloader;
		}
		...
		public InputStream getStream(String imageUri, Object extra) throws IOException {
			...
			switch (Scheme.ofUri(imageUri)) {
				case HTTP:
				case HTTPS:
					return new universalimageloader.core.assist.FlushedInputStream(imageStream);
				default:
					return imageStream;
			}
		}

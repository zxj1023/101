...
//	private List<ImageView> dotList;
	...
	//height 140dp homepgae_bannerview_height
	private void initView() {
		mActionBarHeight = getResources().getDimensionPixelSize(
				R.dimen.aurora_action_bar_search_view_height);

		scrollY = getResources().getDimensionPixelSize(
				R.dimen.homepgae_bannerview_animation_start_point);;
		//滑动到158dp
		bannerViewPagerHeight = getResources().getDimensionPixelSize(
				R.dimen.homepgae_bannerview_animation_distance);

		bannerWidth = getResources().getDimensionPixelSize(
				R.dimen.homepgae_bannerview_width);

		LayoutInflater inflater = LayoutInflater.from(getContext());
		view = inflater.inflate(R.layout.view_framebanner, this);
...
//		bannerDot = (LinearLayout) view.findViewById(R.id.bannerDot);
	 ...
	 * @throws
	 */
	public void setProgress(int dy) {
...
//		int marginTop = (int) (bannerViewPagerHeight * progress) / 2;
		...
		int pivotY = bannerViewPager.getBottom();
		...
		if(dy > bannerViewPagerHeight){
			dy = bannerViewPagerHeight;
		}

		if(scrollY != dy){
			scrollY = dy;
			progress = (scrollY - mActionBarHeight) * 1.0f  / (bannerViewPagerHeight-mActionBarHeight);
			view.setAlpha(1- progress);
			bannerViewPager.setPivotY(pivotY);
			bannerViewPager.setScaleX(1- progress * 0.8f);
			bannerViewPager.setScaleY(1- progress * 0.8f);
		}
	}
	...
	private void initPagerData(int size) {
		...
		bannerViewPager.setPageTransformer(true, new RotationPageTransformer());
...
//					Glide.with(getContext()).load(video_item.get(position).getPicURL())
//							.bitmapTransform(new RoundedCornersTransformation(mContext, 100, 0,
//									RoundedCornersTransformation.CornerType.BOTTOM))
//							.placeholder(R.drawable.banner_loading_default)
//							.error(R.drawable.banner_loading_default).into(iv);
				...
				{
					iv.setImageDrawable(getContext().getResources().getDrawable(R.drawable.default_icon));
				}
			}
...
//							.getPackageName();
					...
					if (bannerType == 1 ) {
						...
						if (!TextUtils.isEmpty(link)) {
							...
							intent.putExtra(AppListActivity.SPECIAL_ID, link);

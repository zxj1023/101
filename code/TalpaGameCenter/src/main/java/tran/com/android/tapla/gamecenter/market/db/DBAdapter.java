...
public class DBAdapter {
	...
	public class DBOpenHelper extends SQLiteOpenHelper {
		...
		//推送数据表
		private static final String DB_CREATE_PUSH  = "CREATE TABLE "
				+ PushDataAdapter.TABLE_NAME + " (" + PushDataAdapter.ID
				+ " INTEGER  not null, "
				+ PushDataAdapter.THEME	+ " TEXT not null, "
				+ PushDataAdapter.SUMMARY + " TEXT  not null, "
				+ PushDataAdapter.ACTION + " INTEGER  not null, "
				+ PushDataAdapter.TO + " TEXT  not null, "
				+ PushDataAdapter.FLAG + " INTEGER , "
				+ PushDataAdapter.STARTTIME + " LONG not null , "
				+ PushDataAdapter.ENDTIME	+ " LONG not null) ";


		//CategoryList table
		private static final String DB_CATEGOTY_LIST  = "CREATE TABLE "
				+ CategoryListDataAdapter.TABLE_NAME + " (" + CategoryListDataAdapter.ID
				+ " INTEGER  not null, "
				+ CategoryListDataAdapter.TITLE		+ " TEXT not null) ";
		...
		public void onCreate(SQLiteDatabase db) {
			...
			db.execSQL(DB_CREATE_PUSH);
			db.execSQL(DB_CATEGOTY_LIST);
		}
		...
		public void onUpgrade(SQLiteDatabase _db, int oldVersion, int newVersion) {
			...
			_db.execSQL("DROP TABLE IF EXISTS " + PushDataAdapter.TABLE_NAME);

			_db.execSQL("DROP TABLE IF EXISTS " + CategoryListDataAdapter.TABLE_NAME);

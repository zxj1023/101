...
import tran.com.android.tapla.gamecenter.market.activity.picbrowser.ImageManager2;
import tran.com.android.tapla.gamecenter.market.activity.picbrowser.PictureInfo;
...
public class PicBrowserAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		ViewHolder vh;
		if (convertView == null) {
			convertView = inflater.inflate(R.layout.pic_browser_gridview_item,
					null);
			vh = new ViewHolder();
			...
			vh.selectedMark = (CheckBox) convertView
					.findViewById(R.id.selectMark);
		...
		if (SystemUtils.isLoadingImage(inflater.getContext())) {
			imageLoader
					.displayImage(vh.image, list.get(position).getimagePath(),
							R.drawable.forum_loading_default, 100, 100);
		}

...
import tran.com.android.tapla.gamecenter.widget.AnimTextView;

public class MiddleProgressBtn extends LinearLayout {
	...
	public MiddleProgressBtn(Context context) {
		super(context);
		initView();
	}

	public MiddleProgressBtn(Context context, AttributeSet attrs) {
		super(context, attrs);
		initView();
	}
	...
	public void setStatus(int status) {
		...
		switch (status) {
			...
			case STATUS_WAIT_DOWNLOAD:
				progressBar.setProgressDrawable(getContext().getDrawable(R.drawable.layer_list_download_btn_wait));
	...
	public void showWaitingAnim(){
		if(status == STATUS_PROGRESSING_INSTALLING){
			progressText.setText(getContext().getString(R.string.download_button_installing));
		}else{
			progressText.setText(getContext().getString(R.string.download_button_wait));
		}

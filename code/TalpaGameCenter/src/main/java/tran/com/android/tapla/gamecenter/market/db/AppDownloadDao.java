...
public class AppDownloadDao {
    ...
    public void insert(DownloadData data, long createTime, long status,
                       long filesize) {
        //确保已完成下载的数据只保持200条，一旦超过，删除最旧的那一条
        if (getDownloadedAppNum() > AppDownloadHelper.LimitDownloadedAppNum) {
            deleteTheOldestDownloadedApp();
        }
        if (db == null) {
            openDatabase();
        }
    ...
    public void updateStatus(int appId, int status) {
        //确保已完成下载的数据只保持200条，一旦超过，删除最旧的那一条
        if ((status > FileDownloader.STATUS_INSTALL_WAIT) && (getDownloadedAppNum() > AppDownloadHelper.LimitDownloadedAppNum)) {
            deleteTheOldestDownloadedApp();
        }
        if (db == null) {
            openDatabase();
     ...
     * @param dir
     */
    public void updateFileDir(int appId, String dir) {
        if (db == null) {
            openDatabase();
        }
        if (db != null) {
     ...
     * @return
     */
    public List<DownloadData> getDownloadedApp(int pageNum, int rowCount) {
        List<DownloadData> data = new ArrayList<DownloadData>();
        if (db == null) {
            openDatabase();
        }
     ...
     * @return
     */
    public long getDownloadedAppNum() {
        if (db == null) {
            openDatabase();
        }
        Cursor cursor = null;
        long count = 0;
        try {
            String sql = "select count(" + AppDownloadHelper.ID + ") from " + TABLE_NAME + " where " + AppDownloadHelper.STATUS + " >= " + FileDownloader.STATUS_INSTALL_WAIT;
     ...
     * @return
     */
    public void deleteTheOldestDownloadedApp() {
        if (db == null) {
            openDatabase();
        }
        if (db != null) {
            Cursor cursor = db.query(TABLE_NAME,
                    new String[]{AppDownloadHelper.ID,
                            AppDownloadHelper.APK_ID,
                            AppDownloadHelper.FILE_NAME,
                            AppDownloadHelper.FILE_DIR},
                    AppDownloadHelper.STATUS + ">=?",
                    new String[]{FileDownloader.STATUS_INSTALL_WAIT + ""}, null,
                    null, AppDownloadHelper.ID + " ASC");
            cursor.moveToNext();
            //apk文件也删除
            String filePath = cursor.getString(3) + File.separator + cursor.getString(2);
            File file = new File(filePath);
            if (file.exists()) {
                file.delete();
            }
            delete(Integer.parseInt(cursor.getString(1)));
            cursor.close();
        }
    }

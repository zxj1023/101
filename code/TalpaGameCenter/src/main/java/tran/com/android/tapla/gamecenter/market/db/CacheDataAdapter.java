...
public class CacheDataAdapter extends DBAdapter {
	...
	public int deleteDataByType(int type) {
		...
		} else if (type == 1000){// 删除除首页和搜索推荐之外的所有数据
			result = mDb.delete(TABLE_NAME, TYPE + " not in (?,?)",
					new String[] { "0","100"});
	...
	private ArrayList<cacheitem> ConvertToCache(Cursor cursor) {
		...
		for (int i = 0; i < resultCounts; i++) {
			...
			m_result.setCat_id(cursor.getString(cursor.getColumnIndex(CAT_ID)));
			m_result.setSpe_id(cursor.getString(cursor.getColumnIndex(SPE_ID)));

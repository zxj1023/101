...
import tran.com.android.gc.lib.widget.AuroraCustomActionBar.onOptionItemClickListener;
...
import tran.com.android.tapla.gamecenter.market.adapter.SpecialAdapter;
...
//		mSearchUtils.initSearchMode(this);
	...
	private void getifCache()
	{
		...
		ldb.open();
		String result = ldb.queryCacheByType(AppListActivity.TYPE_SPECIAL_MAIN,Globals.TYPE_APP,"","");
		if(null != result)
			isCacheData = true;
		ldb.close();
	}
...
//		main_update = (ImageView) mActionBar.findViewById(R.id.actionbar_main_update);
	...
	private void setListener() {
		...
		mListView.setOnScrollListener(new OnScrollListener() {
			...
			public void onScrollStateChanged(AbsListView view, int scrollState) {
				...
				if (obj.getSpecialList().size() < pageNum * rowCount)
					return;
	...
	private void setAdapter() {
		adapter = new SpecialAdapter(this, obj.getSpecialList(), mListView);
					...
					public void run() {
						...
						if (value != null) {
							...
							if (pageNum == 1) {
								...
								int size = value.getSpecialList().size();
								if (size < rowCount)
								...
								for (int i = 0; i < size; i++) {
									obj.getSpecialList().add(
											value.getSpecialList().get(i));
								}

...
import tran.com.android.tapla.gamecenter.market.widget.stickylistheaders.StickyListHeadersAdapter;
...
import static tran.com.android.tapla.gamecenter.market.widget.ProgressBtn.STAUTS_DOWNLOAD_PAUSE;
...
public class DownloadManagerAdapter extends BaseAdapter implements
	...
	private ImageView installFailedIv;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView.findViewById(R.id.aurora_listview_divider)
		...
		case TYPE_DOWNLOADING:
			...
			if (SystemUtils.isLoadingImage(inflater.getContext())) {
				...
				downloadingHolder.iv_icon.setImageResource(R.drawable.default_icon);
			}
			...
			/*downloadingHolder.download_progress.setProgress(bean
					.getProgress());*/
			Log.v("download_progress","download_progress:"+bean.getProgress());
...
//			downloadingHolder.download_progress.setVisibility(View.VISIBLE);
			if (bean.getFileSize() != 0) {
				...
				/*rjz add  start 2017/9/20*/
				downloadingHolder.tv_speed.setVisibility(View.GONE);
				/*rjz add  end 2017/9/20*/
			} else {
				downloadingHolder.tv_progress.setText("");
				/*rjz add start 2017/9/20*/
				downloadingHolder.tv_speed.setVisibility(View.VISIBLE);
				/*rjz add end 2017/9/20*/
			}
			...
			if (status == FileDownloader.STATUS_DOWNLOADING
				...
				downloadingHolder.download_progress_grey.setProgressDrawable(managerActivity.getResources()
						.getDrawable(R.drawable.aurora_progress_color,null));
				/*rjz 注释，不需要两个进度条*/
				/*downloadingHolder.download_progress.setVisibility(View.VISIBLE);*/
...
//						.getString(R.string.downloadman_pause));.
				downloadingHolder.tv_speed.setVisibility(View.VISIBLE);
				downloadingHolder.tv_progress.setVisibility(View.VISIBLE);
				downloadingHolder.btn_operation.setStatus(STAUTS_DOWNLOAD_PAUSE);
...
//								R.color.button_general_text_color));
			} else if (status == FileDownloader.STATUS_WAIT) {	// 按钮状态显示等待
				downloadingHolder.download_progress_grey.setVisibility(View.GONE);
				/*rjz 注释，不需要两个进度条*/
				/*downloadingHolder.download_progress.setVisibility(View.VISIBLE);*/
...
//				downloadingHolder.btn_operation.setText(managerActivity
//						.getString(R.string.downloadman_pause));
				downloadingHolder.btn_operation.setStatus(ProgressBtn.STATUS_WAIT_DOWNLOAD);
				downloadingHolder.tv_speed.setVisibility(View.INVISIBLE);
				downloadingHolder.tv_progress.setVisibility(View.INVISIBLE);
				downloadingHolder.tv_speed.setTextColor(managerActivity
						.getResources().getColor(
								R.color.down_man_item_status_text));
				downloadingHolder.tv_speed.setText(getStatusString(status,
...
//								R.color.button_default_text_color));
				...
				downloadingHolder.tv_speed.setTextColor(managerActivity
						.getResources().getColor(
								R.color.aurora_red_text_color));
//				downloadingHolder.btn_operation.setText(managerActivity
//						.getString(R.string.downloadman_retry));
				downloadingHolder.btn_operation.setStatus(ProgressBtn.STATUS_DOWNLOAD_FAIL);
//				downloadingHolder.btn_operation
...
//								R.color.button_default_text_color));
				...
				downloadingHolder.tv_speed.setTextColor(managerActivity
						.getResources().getColor(
								R.color.down_man_item_status_text));
				downloadingHolder.download_progress_grey.setProgressDrawable(managerActivity.getResources()
				.getDrawable(R.drawable.grey_progressbar,null));
//				downloadingHolder.btn_operation.setText(managerActivity
//						.getString(R.string.downloadman_continue));
				downloadingHolder.btn_operation.setStatus(ProgressBtn.STAUTS_DOWNLOAD_GO_ON);
//				downloadingHolder.btn_operation
//						.setBackgroundResource(R.drawable.button_default_selector);
//				downloadingHolder.btn_operation.setTextColor(managerActivity
//						.getResources().getColorStateList(
//								R.color.button_default_text_color));
			...
			downloadingHolder.btn_operation.setOnButtonClickListener(clickListener);
			break;
		case TYPE_DOWNLOADED:
			if (bean.getDownloadData() == null) {
				...
				convertView.findViewById(
						R.id.aurora_item_sliding_switch)
						.setVisibility(View.VISIBLE);
			} else {
				...
				if (SystemUtils.isLoadingImage(inflater.getContext())) {
					...
					downloadedHolder.iv_icon.setImageResource(R.drawable.default_icon);
				}
				...
				downloadedHolder.tv_status.setTextColor(
						managerActivity.getResources().getColor(R.color.down_man_item_status_text,null));
				downloadedHolder.installFailedIv.setVisibility(View.GONE);
				if (installedAppInfo != null
					...
					downloadedHolder.progressBtn.setStatus(ProgressBtn.STATUS_FOUCE);
				} else { // 显示安装
					if (bean.getDownloadStatus() == FileDownloader.STATUS_INSTALLING) {
						downloadedHolder.progressBtn.setStatus(ProgressBtn.STATUS_PROGRESSING_INSTALLING);
						downloadedHolder.tv_status.setText(managerActivity
								.getString(R.string.downloadman_installing));
					} else if(bean.getDownloadStatus()==FileDownloader.STATUS_INSTALLFAILED){
						downloadedHolder.installFailedIv.setVisibility(View.VISIBLE);
						downloadedHolder.progressBtn.setStatus(ProgressBtn.STATUS_DOWNLOAD_FAIL);
						downloadedHolder.tv_status.setTextColor(
								managerActivity.getResources().getColor(R.color.aurora_red_text_color,null));
//						downloadedHolder.tv_status.setText(managerActivity
	...
	public View getHeaderView(int position, View convertView, ViewGroup parent) {
		...
		if (sectionIndices.length > 1) {
			...
			if (id == DownloadManagerBean.TYPE_DOWNLOADING) {
				String titleStr=downloadingStr + "（" + downloadingCount + "）";
				SpannableStringBuilder spannableStringBuilder=Utils.setTVColor(titleStr, "（", "）"
						,managerActivity.getResources().getColor(R.color.aurora_red_text_color,null));
				headerHolder.tv_title.setText(spannableStringBuilder);
				headerHolder.iv_clean.setVisibility(View.GONE);
			} else {
				String titleStr=downloadedStr + "（" + downloadedCount + "）";
//				SpannableStringBuilder spannableStringBuilder=Utils.setTVColor(titleStr, "（", "）", Color.RED);
				headerHolder.tv_title.setText(titleStr);
...
//		ProgressBar download_progress;/*rjz 注释，不需要两个进度条*/
		TextView tv_speed;
		TextView tv_progress;
		ProgressBtn btn_operation;
	}
	...
	private void findBaseHolder(View view, BaseHolder baseHolder) {
		...
		baseHolder.cb = (AuroraCheckBox) view
				.findViewById(R.id.aurora_list_left_checkbox);
	}
...
//				.findViewById(R.id.download_progress);
		...
		downloadingHolder.btn_operation = (ProgressBtn) view
	...
	private DownloadedHolder createDownloadedHolder(View view) {
		...
		downloadedHolder.installFailedIv = (ImageView) view.findViewById(R.id.install_failed_iv);
...
//					downloadingHolder.btn_operation.setText(managerActivity
//							.getString(R.string.downloadman_pause));
					downloadingHolder.btn_operation.setStatus(ProgressBtn.STAUTS_DOWNLOAD_PAUSE);
					downloadingHolder.tv_speed.setTextColor(managerActivity
							.getResources().getColor(

...
import tran.com.android.tapla.gamecenter.market.db.AutoUpdateDao;
...
public class AutoUpdateService extends Service {
    ...
    public static final int OPERATION_AUTO_UPDATE_CHECK = 106;    // 自动检查 chenjie
    ...
    private static final int HANDEL_AUTO_CHECK = 203;//自动检查 chenjie
	...
	private class MyBroadcastReciver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
            ...
            LogUtils.logd(TAG, "zhangwei the action=" + action);
	...
	private Handler mHandle = new Handler() {
		...
		public void handleMessage(Message msg) {
			switch(msg.what) {
			case HANDEL_CHECK:
                    LogUtils.logd(TAG, "AutoUpdateService HANDEL_CHECK");
				downloading = false;
                ...
                case HANDEL_AUTO_UPDATE:
                    ...
                    LogUtils.logd(TAG, "HANDEL_AUTO_UPDATE udateList="+udateList.size());
                    if (null == downloaders) {
                        LogUtils.logd(TAG, "HANDEL_AUTO_UPDATE downloaders null");
                        return;
                    }
                    ...
                    for (DownloadData data : udateList) {
                        ...
                        if (!AppDownloadService.getDownloaders().containsKey(data.getApkId())) {
                            if (!downloaders.containsKey(data.getApkId())) {
                                ...
                                AppDownloadService.startDownload(AutoUpdateService.this, data);//Start Download by chenjie
                                LogUtils.logd(TAG, "Auto Downloader --download");
                            }
                        }
                    }
            /*	for (int key : downloaders.keySet()) {
...
//				context = null;
                    break;
                case HANDEL_AUTO_CHECK:
                    LogUtils.logd(TAG, "HANDEL_AUTO_CHECK");
                    List<DownloadData> checkList = (List<DownloadData>) msg.obj;
                    if (null == downloaders) {
                        LogUtils.logd(TAG, "HANDEL_AUTO_CHECK");
                        return;
                    }
                    ...
                    for (int key : downloaders.keySet()) {
                        FileDownloader downloader = downloaders.get(key);
                        boolean exists = false;
                        for (DownloadData data : checkList) {
                            if (downloader.getDownloadData().getApkId() == data.getApkId()) {
                                exists = true;
                                break;
                            }
                    ...
                    LogUtils.logd(TAG, "downloaders.size()=" + downloaders.size());
    ...
    private void handleOperation(int operation) {
        switch (operation) {
            ...
            case OPERATION_AUTO_UPDATE:
                ...
                break;
            case OPERATION_AUTO_UPDATE_CHECK:
                new Thread() {
                    @Override
                    public void run() {
                        LogUtils.logd(TAG, "run thread");
                        ...
                        List<DownloadData> udateList = dataFromUtils.getUpdateData(context);
                        if (udateList != null && udateList.size() > 0) {
                            mHandle.sendMessage(mHandle.obtainMessage(HANDEL_AUTO_CHECK, udateList));
                        }
                    }
                }.start();
...
//=================================已下为public方法=================================//
	...
	public static void startAutoUpdate(Context context,int type) {
        LogUtils.logd(TAG, "run startAutoUpdate()");
        ...
        if ((type == 0) && UpdateSettingsPreferenceActivity.getPreferenceValue(context,
            ...
            Bundle startDownloadBundle = new Bundle();
            startDownloadBundle.putInt(AutoUpdateService.DOWNLOAD_OPERATION,
                    AutoUpdateService.OPERATION_AUTO_UPDATE_CHECK);
            startDownload.putExtras(startDownloadBundle);
            context.startService(startDownload);
		}
    }
    ...
    private void sendUpdateNotify() {
        ...
        PendingIntent pIntent = PendingIntent.getActivity(context, 101,
                intent, PendingIntent.FLAG_UPDATE_CURRENT);

        builder.setSmallIcon(android.R.drawable.stat_sys_download);
        builder.setContentIntent(pIntent);
        builder.setContentTitle(getResources().getString(R.string.notification_status_update));
        builder.setContentText(downloaders.size()+""+getResources().getString(R.string.notification_status_dis_update));
    ...
    public static void pauseAutoUpdate() {
        LogUtils.logd(TAG, "run pauseAutoUpdate()");
    ...
    public static void continueAutoUpdate(Context context) {
        LogUtils.logd(TAG, "run continueAutoUpdate()");
        ...
        if (UpdateSettingsPreferenceActivity.getPreferenceValue(context,
            ...
            LogUtils.logd(TAG, "run continueAutoUpdate()1");
            Intent startDownload = new Intent(context, AutoUpdateService.class);
            Bundle startDownloadBundle = new Bundle();
            startDownloadBundle.putInt(AutoUpdateService.DOWNLOAD_OPERATION,
    ...
    public static void stopAutoUpdate(Context context) {
        LogUtils.logd(TAG, "run stopAutoUpdate()");

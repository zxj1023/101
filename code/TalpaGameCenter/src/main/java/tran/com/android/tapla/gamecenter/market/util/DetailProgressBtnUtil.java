...
public class DetailProgressBtnUtil {

	public static final String TAG = "ProgressBtnUtil";

	public void updateProgressBtn(DetailProgressBtn btn, DownloadData data) {
		updateProgressBtn(btn, data, null);
	}

	public void updateProgressBtn(DetailProgressBtn btn, DownloadData data, OnClickListener onClickListener) {
		// 检测是否安装
					...
					if (tempData.getVersionCode() == data.getVersionCode()) {
						...
						if (status == FileDownloader.STATUS_INSTALL_WAIT) {		// 等待安装
							...
							if (id == tempData.getApkId() && status == FileDownloader.STATUS_INSTALLED
									&& btn.getStatus() == DetailProgressBtn.STATUS_PROGRESSING_INSTALLING) {
								return;
							}
	...
	public void updateFinishProgressBtn(DetailProgressBtn btn, DownloadData data) {
		// 检测是否安装
		InstalledAppInfo installedAppInfo = InstallAppManager
				.getInstalledAppInfo(btn.getContext(),
						data.getPackageName());
	 ...
	 * 显示下载操作
	 */
	private void showOperationDownload(final DetailProgressBtn progressBtn,
		...
		progressBtn.setStatus(DetailProgressBtn.STATUS_NORMAL);
		progressBtn.setOnNormalClickListener(clickListener);
		final int apkId = downloadData.getApkId();
		progressBtn.setOnBeginAnimListener(new DetailProgressBtn.OnAnimListener() {
			@Override
			public void onEnd(DetailProgressBtn view) {
				FileDownloader downloader = AppDownloadService.getDownloaders()
						.get(apkId);
				if (downloader != null) {
					int status = downloader.getStatus();
					if (status == FileDownloader.STATUS_CONNECTING
						...
						view.setStatus(DetailProgressBtn.STATUS_PROGRESSING_DOWNLOAD);
					}
				}
			}
		});
	 ...
	 * 显示更新操作
	 */
	private void showOperationUpdate(final DetailProgressBtn progressBtn,
	 ...
	 *
	 */
	private void showOperationDownloading(final DetailProgressBtn progressBtn,
		...
		if (status == FileDownloader.STATUS_WAIT) {
			if (!progressBtn.isRuningStartAnim()) {
				progressBtn.setStatus(DetailProgressBtn.STATUS_WAIT_DOWNLOAD);
			}
		} else {
			if (!progressBtn.isRuningStartAnim()) {
				progressBtn.setStatus(DetailProgressBtn.STATUS_PROGRESSING_DOWNLOAD);
			}
	 ...
	 *
	 */
	private void showOperationContinue(final DetailProgressBtn progressBtn,
			final FileDownloader downloader) {
		OnClickListener clickListener = new OnClickListener() {
			@Override
			public void onClick(View v) {
		...
		progressBtn.setStatus(DetailProgressBtn.STAUTS_DOWNLOAD_CONTINUE);
		progressBtn.setOnProgressClickListener(clickListener);
	}

	private void doOperationContinue(final DetailProgressBtn progressBtn,
			final FileDownloader downloader)
	{
		int status = downloader.getStatus();
	 ...
	 *
	 */
	private void showOperationRetry(final DetailProgressBtn progressBtn,
			final FileDownloader downloader) {
		OnClickListener clickListener = new OnClickListener() {
			@Override
			public void onClick(View v) {
		...
		progressBtn.setStatus(DetailProgressBtn.STATUS_DOWNLOAD_FAIL);
		progressBtn.setProgress(progress);
		progressBtn.setOnProgressClickListener(clickListener);
	 ...
	 *
	 */
	private void showOperationInstall(final DetailProgressBtn progressBtn,
		...
		if (id == downloadData.getApkId()) {
			if (!progressBtn.isRuningEndAnim()) {
				if (progressBtn.getStatus() == DetailProgressBtn.STATUS_PROGRESSING_INSTALLING
						|| progressBtn.getStatus() == DetailProgressBtn.STATUS_WAIT_INSTALL) {
					progressBtn.startEndAnim(false);
				} else {
					progressBtn.setStatus(DetailProgressBtn.STATUS_FOUCE_NORMAL);
				}
			}
		} else {
			progressBtn.setStatus(DetailProgressBtn.STATUS_FOUCE_NORMAL);
	 ...
	 *
	 */
	private void showWaitInstall(final DetailProgressBtn progressBtn,
			final DownloadData downloadData) {
		// ProgressBtn
		progressBtn.setStatus(DetailProgressBtn.STATUS_WAIT_INSTALL);
	}
	 ...
	 *
	 */
	private void showInstalling(final DetailProgressBtn progressBtn,
			final DownloadData downloadData) {
		Log.i(TAG, downloadData.getApkName() + " showInstalling");

		// ProgressBtn
		progressBtn.setStatus(DetailProgressBtn.STATUS_PROGRESSING_INSTALLING);
	}
	 ...
	 *
	 */
	private void showOperationOpen(final DetailProgressBtn progressBtn,
			final DownloadData downloadData) {
		OnClickListener clickListener = new OnClickListener() {
			@Override
			public void onClick(View v) {
		...
		if (id == downloadData.getApkId()) {
			if (!progressBtn.isRuningEndAnim()) {
				if (progressBtn.getStatus() == DetailProgressBtn.STATUS_PROGRESSING_INSTALLING
					...
					progressBtn.setStatus(DetailProgressBtn.STATUS_FOUCE);
				}
			}
		} else {
			progressBtn.setStatus(DetailProgressBtn.STATUS_FOUCE);

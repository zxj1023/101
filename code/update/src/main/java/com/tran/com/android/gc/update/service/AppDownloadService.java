...
//			cancleNotify(currentDownloadData.getApkId());
    ...
    private void download(FileDownloader downloader) {
        ...
        if (AutoUpdateService.getDownloaders() != null &&
            ...
            //AutoUpdateService.startAutoUpdate(context, 2);
        }
    ...
    public static Map<Integer, FileDownloader> getDownloaders() {
        if (null == downloaders) {
            downloaders = new ConcurrentHashMap<Integer, FileDownloader>();
            if (updateApp.getInstance().getApplicationContext() != null) {
                Intent i = new Intent(updateApp.getInstance().getApplicationContext(), AppDownloadService.class);
                updateApp.getInstance().getApplicationContext().startService(i);
            }
        }
        return downloaders;
    }

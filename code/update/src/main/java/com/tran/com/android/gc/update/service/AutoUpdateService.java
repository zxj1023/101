package com.tran.com.android.gc.update.service;
...
import com.tran.com.android.gc.update.activity.setting.UpdateSettingsPreferenceActivity;
import com.tran.com.android.gc.update.db.AutoUpdateDao;
import com.tran.com.android.gc.update.download.DownloadStatusListener;
import com.tran.com.android.gc.update.download.FileDownloader;
import com.tran.com.android.gc.update.download.FilePathUtil;
...
public class AutoUpdateService extends Service {
    ...
    public static final int OPERATION_FORCE_UPDATE = 96;   // 强制更新
    ...
    public void onCreate() {
        ...
        Log.i("Test", "AutoUpdateService oncreate");
        if (isNeedInitData()) {
            context = this;
                                ...
                                DownloadStatusListener listener = new DownloadStatusListener() {
                                    ...
                                    public void onDownload(int appId, int status,
                                        ...
                                        if (status == FileDownloader.STATUS_INSTALL_WAIT
                                            ...
                                            AppInstallService.startInstall(context,
                                                    autoUpdateDao.getDownloadData(appId),
                                                    AppInstallService.TYPE_AUTO_UPDATE);
                                        }
                                    }
                                };
                                if (data.getForceUpFlag() == 1) {
                                    FileDownloader fileDownloader = new FileDownloader(data, dir,
                                            listener, FileDownloader.TYPE_FORCE_UPDATE);
...
//				context = null;
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        Log.i("Test", "AutoUpdateService onStartCommand");
...
//						// 当下载完成时, 发送下载完成广播
     ...
     * @return
     */
    private void continueForceUpdate() {
        if (downloaders != null && downloaders.size() > 0) {
            List<FileDownloader> list = new ArrayList<FileDownloader>();
            for (int key : downloaders.keySet()) {
                FileDownloader downloader = downloaders.get(key);
            ...
            for (FileDownloader downloader : list) {
                if (downloader.getType() == FileDownloader.TYPE_FORCE_UPDATE) {
                    download(downloader);
                }
            }
        }
...
//=================================已下为public方法=================================//
    ...
    public static void updateDownloadList(Context context) {
        Log.i(TAG, "run dateDownloadList()");

//        SharedPreferences sp = context.getSharedPreferences(
//                Globals.SHARED_DATA_UPDATE, MODE_APPEND);
//        String time = sp.getString(Globals.SHARED_APP_LIST_UPDATETIME, "0");
...
//            editor.putString(Globals.SHARED_APP_LIST_UPDATETIME, TimeUtils.getStringDateShort());
//            editor.commit();
        }
    }

    public static void startMyself(Context context) {
        com.tran.com.android.gc.update.util.Log.i("Test", "startMyself start");

        Intent startDownload = new Intent(context, AutoUpdateService.class);
        ComponentName cn = context.startService(startDownload);
        if (cn == null) {
            Log.i("Test", "ComponentName = " + cn);
        }

        com.tran.com.android.gc.update.util.Log.i("Test", "startMyself end");
    }
    ...
    public static void continueAutoUpdate(Context context) {
        Log.i(TAG, "run continueAutoUpdateDev()");
        ...
        if (SystemUtils.isWifiNetwork(context) && (ifKeyG)) {
            if (UpdateSettingsPreferenceActivity.getPreferenceValue(context,
                ...
                Bundle startDownloadBundle = new Bundle();
                startDownloadBundle.putInt(AutoUpdateService.DOWNLOAD_OPERATION,
                        AutoUpdateService.OPERATION_FORCE_UPDATE);
                startDownload.putExtras(startDownloadBundle);
                context.startService(startDownload);
            }
        }
